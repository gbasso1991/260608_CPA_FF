...
for p in temps_500_CPA_control_LN2:
    print('  -',i,p)
    i+=1
...
for p in temps_500_CPA_control_LN2:
    _,time,temp_CH1, _ = lector_templog(p)
    ax2.plot(time,temp_CH1,label=os.path.basename(p).split('.')[0])
ax2.set_title('2 - CPA control - LN2 --> BT')
ax2.set_xlim(0,)
ax2.grid()
ax2.legend()
ax2.set_ylabel('T (°C)')
ax2.set_xlabel('t (s)')
#%%3  500 uL EG60 Agua40 LN2
print('-'*50,'\n500 uL 60% Etilenglicol 40% agua - Enfriamiento en LN2 - Calentamiento en BT','\n')
temps_500_EG60_agua40_LN2 = glob("data/*500uL_EG60_agua40_LN2*",recursive=True)
temps_500_EG60_agua40_LN2.sort()
for p in temps_500_EG60_agua40_LN2:
    print('  -',i,p)
    i+=1

fig3,ax3 = plt.subplots(figsize=(9,4.5),constrained_layout=True)
for p in temps_500_EG60_agua40_LN2:
    _,time,temp_CH1, _ = lector_templog(p)
    ax3.plot(time,temp_CH1,label=os.path.basename(p).split('.')[0])
ax3.set_title('3 - 60% Etilenglicol 40% agua - LN2 --> BT')
ax3.set_xlim(0,)
ax3.grid()
ax3.legend()
ax3.set_ylabel('T (°C)')
